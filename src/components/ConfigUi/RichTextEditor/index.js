...
export class RichTextEditor extends React.Component {
  ...
  toggleAddImageUrlBox = value =>
    this.setState(
      () => (!value ? { isAddImageUrlBoxOpen: !this.state.isAddImageUrlBoxOpen } : { isAddImageUrlBoxOpen: value })
    );
  ...
  toggleText = e => {
    e.preventDefault();
    const isStringInHtmlFormat = str => str.replace(/(<([^>]+)>)/gi, '') !== str;
    const { editorState } = this.state;
