...
class NestedListInput extends Component {
  ...
  onDragEnd = result => {
    ...
    if (result.type !== 'DEFAULT') {
      ...
      const updatedReasons = this.props.selectedEntityId !== 'create' ? reOrderedReasons.map((reason, index) => reason.set('sortOrder', index)) : reOrderedReasons;
              ...
              {(provided, snapshot) => (
                ...
                >
                  {this.props.reasonHeaders.map((category, index) => {
                    let hierarchy = category.get('hierarchy').size > 0 ? category.get('hierarchy').toJS()[0] : 'default';
                    return (
                      <Draggable
                                      ...
                                      >
                                        {(provided, snapshot) => (
                                          ...
                                          >
                                            ...
                                            <ReasonItem title={reason.get('name')}>
                                              {reason.get('name')}
                                            </ReasonItem>
