...
class ListInput extends Component {
    ...
    addFormItem = e => {
      ...
      if(this.state.currentListItem.trim() !== '') {
        this.props.input.onChange(this.props.input.value === ''? new List([this.state.currentListItem]) : this.props.input.value.push(this.state.currentListItem))
      }
    ...
    render() {
      return (
        ...
        >
          ...
          <InputWrapper className="list-item-add-input" onChange={this.saveRefrence} value={this.state.currentListItem} />
          <ActionButton className="list-item-add-button" onClick={this.addFormItem}>
              <PlusIconSVG size={12} />
          </ActionButton>
                  ...
                  (<Draggable draggableId={li} index={index} key={li + index}>
                    {(provided) => (
                      <ListItem innerRef={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                        {this.props.input.name !== 'urls' && <Grip className="list-item-grip-icon" type={this.props.input.name}>:::</Grip>}
                        <ListItemText className="list-item-text" type={this.props.input.name}> {li} </ListItemText>
                        <RemoveButton className="list-item-remove-button" type={this.props.input.name} onClick={() => this.removeListItem(index)}>
