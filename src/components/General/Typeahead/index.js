...
${props => props.isActiveSuggestion? `background: #00dfff7d !important;` : ''}
&:hover {
...
updateSuggestions = () => {
  ...
  const suggestionTimerId = setTimeout(() => {
    ...
    const suggestions = this.props.options.filter(
      option =>
        option.label.toLowerCase().indexOf(compareVal) > -1
    );
    this.setState({
      activeSuggestion: 0,
      suggestions,
    ...
    <div
      ...
      className={this.props.className}>
        ...
        <SearchIconWrapper 
          ...
          inputWidth={this.props.listWidth}/>
      {this.state.hasFocus && 
          ...
          width={this.props.listWidth}>
            ...
            {this.state.suggestions.length === 0 &&
              <NoSuggestionsMessage>
                {this.props.noSuggestionsMessage}
              </NoSuggestionsMessage>}
      </SuggestionsContainer>}
    </div>
  );
